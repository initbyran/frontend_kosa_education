<template>
  <div id="app">
    ...
    <BoxOfficeList
      v-bind:propsdata="myData"
      v-bind:propsdataimg="movieImg"
      v-bind:propsdatadetail="myDetails"
      v-on:deleteSelectedBtn="deleteSelectedBtn"
      v-on:myModal="myModal"
    ></BoxOfficeList>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    deleteSelectedBtn(selectedMovies) {
      selectedMovies.forEach((el, idx) => {
        if (el) {
          this.myData = this.myData.filter(
            (item) => parseInt(item.rnum) - 1 !== idx
          );
        }
      });
    },

    myModal: function (movieCd) {
      axios({
